...
class ReviewController extends Controller
{
    public function add($product_slug)
    {
        ...
        {
            return redirect()->back()->with('status', "Acest link nu exista");
        }
    }
    ...
    public function create(Request $request)
    {
        ...
        if($product)
        {
            ...
            if($new_review)
            {
                return redirect('category/'.$category_slug.'/'.$prod_slug)->with('status', "Multumim pentru feedback");
            }
        }
        else
        {
    ...
    public function update(Request $request)
    {
        ...
        if($user_review != '')
        {
            ...
            if($review)
            {
                ...
                $review->update();
                return redirect('category/'.$review->product->category->slug.'/'.$review->product->slug)->with('status', "Review actualizat cu succes");
            }
            else
            {
                return redirect()->back()->with('status', "Acest link nu exista");
            ...
            {
                return redirect()->back()->with('status', "Nu poti submite un review gol");
            }
    }
}
